...
class Joy:
    ...
    def open(self) -> bool:
        try:
            ...
            print(f"Failed to open joystick: {self.path} ({ex})", file=sys.stderr)
            return False
        nm = joy_name(self.fd)
        if nm:
            print(f"Joystick: {nm} ({self.path})")
        return True
        ...
        while True:
            try:
                ...
                if ex.errno in (errno.EAGAIN, errno.EWOULDBLOCK):
                    break
                print("Joystick read error", file=sys.stderr)
                break
            if len(data) < 8:
                break
...
def main() -> int:
    ...
    if len(sys.argv) < 2:
        print(f"Usage: python3 {sys.argv[0]} <NIC> [joystick-path]", file=sys.stderr)
        return 1

    jpath = pick_joy_path(sys.argv)
    if not jpath:
        print("No joystick found. Try: ls /dev/input/js*", file=sys.stderr)
        for p in list_js_nodes():
