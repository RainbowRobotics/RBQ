...
if [[ "$NO_CACHE" == "true" ]]; then
    ...
    rm -rf "$DOCKER_DIR" 2>/dev/null || sudo rm -rf "$DOCKER_DIR"
fi
...
if [[ "$NO_CHECK" == "false" ]]; then
    ...
    $DOCKER build --file scripts/docker/Dockerfile --network host -t $IMAGE_NAME ..
fi
CMD="bash scripts/build.bash ${CMD_ARGS[@]}"
mkdir -p "$DOCKER_DIR/bin" "$DOCKER_DIR/build"
$DOCKER run \
    --rm \
    --user $(id -u):$(id -g) \
    --cap-add SYS_ADMIN \
