...
void Simulate::LoadOnRenderThread()
{
    ...
    if (!this->is_passive_)
    {
        ...
        int history_length = mjMIN(INT_MAX / state_bytes, 10000);
        ...
        mju::strcmp_arr(this->filename, this->previous_filename))
    {
        ...
        // 카메라를 더 멀리서 시작하도록 설정 (AlignAndScaleView 후에 적용)
        this->cam.distance = 5.0;  // 기본값보다 훨씬 멀리
        this->cam.elevation = -20; // 약간 아래에서 바라보도록 설정
        
        // base_link 본체를 자동으로 추적하도록 설정
        int base_body_id = mj_name2id(this->m_, mjOBJ_BODY, "base_link");
        if (base_body_id > 0) {
            this->cam.type = mjCAMERA_TRACKING;
            this->cam.trackbodyid = base_body_id;
            this->cam.fixedcamid = -1;
            this->camera = 1; // UI에서 "Tracking" 모드로 설정
        }
    }
...
void Simulate::RenderLoop()
{
    ...
    // `opt` aliases the caller's mjvOption (ctor init list); main() already defaults and then
    // configures it. Re-defaulting here reverted the geomgroup, so --livox showed nothing.
