...
print_help() {
    ...
    echo ""
    echo "Anything else is forwarded to the simulator, e.g. --wheel for the wheeled RBQ."
}
...
# --- Host docker setup (sudo) — only when docker is NOT usable without sudo (fresh machine).
# Skipped where docker already works: sudo needs a TTY, and this would remove a docker.io
# that the environment may depend on.
if [[ -n "$SUDO" ]]; then
...
# --- NVIDIA Container Toolkit install --- (only with an NVIDIA GPU, same condition as
# "GPU support" below, so machines without one never hit the sudo install)
if command -v nvidia-smi &>/dev/null && nvidia-smi &>/dev/null \
   && ! dpkg -s nvidia-container-toolkit &>/dev/null; then
...
# --- X11 / GUI ---
# Only touched for interactive runs: --build/--test need no host X11 (--test renders via xvfb).
# xhost can hang forever against an inherited display it cannot reach, so it is also timed out.
DISPLAY_ARGS=()
...
if [[ "$TEST_ONLY" == "true" ]]; then
    ...
    # GLFW needs an X display even offscreen, so render under Xvfb. Docker options must
    # precede the image name; --init reaps Xvfb so the container exits cleanly. Non-interactive.
    set +e
    $DOCKER run --rm --init \
        --cap-add SYS_ADMIN \
        --network host \
        ...
        --entrypoint xvfb-run \
        "$DOCKER_NAME" \
        -a /workspace/bin/rbq_mujoco "${SIM_ARGS[@]}" --selftest "$SELFTEST_SEC"
    STATUS=$?
    set -e
    if [[ $STATUS -eq 0 ]]; then
