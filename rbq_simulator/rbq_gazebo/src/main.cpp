...
namespace rbq_gazebo_system { bool addToServer(gz::sim::Server &server); }
...
// Per-world spawn pose: x y z roll pitch yaw.
void spawnPose(const std::string &world, double out[6])
{
    struct P { const char *w; double v[6]; };
    static const P table[] = {
        {"empty",     {0.0,   0.0,  0.70, 0.0, 0.0, 0.0 }},
    };
    for (const auto &p : table) {
        if (world == p.w) { std::memcpy(out, p.v, sizeof(p.v)); return; }
    }
    const double def[6] = {0.0, 0.0, 0.30, 0.0, 0.0, 0.0};
    std::memcpy(out, def, sizeof(def));
}
...
bool spawnRobot(gz::transport::Node &node, const std::string &worldName,
                const std::string &robotUrdfPath, const double pose[6])
{
...
int main(int argc, char *argv[])
{
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (a == "--world" && i + 1 < argc) world = argv[++i];
            ...
            std::printf("Usage: Gazebo [--world <name>] [--livox|--ouster] "
                        "[-i <iface>] [--headless]\n"
    ...
    const fs::path robotUrdfPath = resourcesDir / "models" / "rbq10/rbq.urdf";
    ...
    std::printf("[Gazebo] world=%s (%s), lidar=%s, headless=%d\n",
                worldName.c_str(), worldSdf.c_str(), lidar.c_str(), headless);
    ...
    // Register RBQ plugin in-process before Run(); Configure() detects rbq10 by scanning the
    // ECM in PreUpdate.
    if (!rbq_gazebo_system::addToServer(server)) {
    ...
    double pose[6];
    spawnPose(worldName, pose);
    if (!spawnRobot(node, worldName, spawnUrdf.string(), pose)) {
        std::fprintf(stderr, "[Gazebo] robot spawn failed\n");
        return 1;
    }
    std::printf("[Gazebo] rbq10 spawned.\n");
    ...
    // Start threads.
    std::thread bridgeT(rbq_gazebo::bridgeLoop, worldName, std::ref(g_running), rbq_gazebo::legJoints());
    // Camera thread
