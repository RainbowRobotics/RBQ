...
{
public:
  // rbq10 joint config (12 legs).
  std::vector<rbq_gazebo::JointSpec> jointSpecs{rbq_gazebo::legJoints()};
  int numJoints{static_cast<int>(jointSpecs.size())};
  ...
  gz::sim::EntityComponentManager &_ecm)
{
  ...
  std::fprintf(stdout, "[rbq_gazebo][INFO] " "Found robot model 'rbq10' (%d joints)\n", this->numJoints);
  ...
  for (const auto &spec : this->jointSpecs) {
    if (findJoint(spec.name) != gz::sim::kNullEntity) {
      this->jointNames.push_back(spec.name);
    } else {
  ...
  if (this->wrapper->init(&_ecm, this->jointNames, this->imuEntity)) {
  ...
  for (size_t i = 0; i < this->jointNames.size(); ++i) {
    this->wrapper->resetJointState(i, this->jointSpecs[i].holdPos, 0.0);  // spawn pose
  ...
  gz::sim::EventManager &/*_eventMgr*/)
{
  ...
  std::fprintf(stdout, "[rbq_gazebo][INFO] " "Plugin loaded. Waiting for robot model 'rbq10'...\n");
}
