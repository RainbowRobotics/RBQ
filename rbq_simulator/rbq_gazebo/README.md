...
The fragment also carries the housing's `<collision>` and `<inertial>`. sdformat lumps fixed-joint
children into the trunk, so both land on the trunk body — Gazebo and the MuJoCo model then agree on
mass, centre of mass and inertia to the last digit (17.288 kg with `--livox`, 17.205 kg with
`--ouster`). Without them the LiDAR, which is the tallest part of the robot, would exist only to the
renderer.
