...
print_help() {
    ...
    echo "  -r, --robot <name>      Specify robot name (e.g. rb1, wheel)"
...
if [ "$MOTION_ENABLED" = "true" ]; then
    gnome-terminal --tab --title="Motion"   -- bash -i -c 'bash scripts/start_motion.bash --sim "$@"' bash "${IFACE_ARGS[@]}" "${ROBOT_ARGS[@]}"
fi
if [ "$SIM_ENABLED" = "true" ]; then
    gnome-terminal --tab --title="Mujoco"   -- bash -i -c 'bash scripts/start_mujoco.bash "$@"' bash "${IFACE_ARGS[@]}" "${SIM_ARGS[@]}" "${ROBOT_ARGS[@]}" 
fi
